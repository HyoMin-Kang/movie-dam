...
public class BoardDBBean {
	...
	public int getArticleCount(String option, String search)
             throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        ...
        try {
            ...
            if(option==null) {
            	...
            	pstmt = conn.prepareStatement(
            			"select count(*) from board where writer LIKE '%"+search+"%'");
            }else if(option.equals("subject")) {
            	pstmt = conn.prepareStatement(
            			"select count(*) from board where subject LIKE '%"+search+"%'");
            }else if(option.equals("content")) {
            	pstmt = conn.prepareStatement(
            			"select count(*) from board where content LIKE '%"+search+"%'");
            }
	...
	//글의 목록을 가져옴
	public List<BoardDataBean> getArticles(int start, int end, String option, String search)
             throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        ...
        try {
            ...
            if(option==null) {
            	...
            	pstmt = conn.prepareStatement(
            			sql+="where writer LIKE '%"+search+"%' order by ref desc, re_step asc limit ?,? ");
                pstmt.setInt(1, start-1);
    			pstmt.setInt(2, end);
            }else if(option.equals("subject")) {
            	pstmt = conn.prepareStatement(
            			sql+="where subject LIKE '%"+search+"%' order by ref desc, re_step asc limit ?,? ");
                pstmt.setInt(1, start-1);
    			pstmt.setInt(2, end);
            }else if(option.equals("content")) {
            	pstmt = conn.prepareStatement(
            			sql+="where content LIKE '%"+search+"%' order by ref desc, re_step asc limit ?,? ");
                pstmt.setInt(1, start-1);
    			pstmt.setInt(2, end);
            }
