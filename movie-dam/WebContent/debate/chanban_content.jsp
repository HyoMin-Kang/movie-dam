...
<%
	...
	try {
		...
		String textWithoutTag = chanban.getCb_content()
	...
	<div class="container">
			...
			<div class="col-10">
				<div class="single-listing-content">
					<div class="listing-title">
						...
						<span>작성자 <a href="/movie-dam/member/profile.jsp?mem_userid=<%=chanban.getCb_writer()%>"><%=chanban.getCb_writer()%></a>&nbsp;(<%=sdf.format(chanban.getReg_date())%>)
						</span> <span><i class="fas fa-eye"></i> <%=chanban.getCb_hits()%></span>
					</div>
					<hr>
		...
		<div class="container">
			<form action="chanban_contentPro.jsp">
				<input type="hidden" name="cb_cmt_level" value="<%=cb_cmt_level%>"> <input type="hidden" name="cb_cmt_group" value="<%=cb_cmt_group%>"> <input type="hidden" name="cb_cmt_step" value="<%=cb_cmt_step%>"> <input type="hidden" name="cb_cmt_level" value="<%=cb_cmt_level%>"> <input type="hidden" name="cb_cmt_ref" value="<%=chanban.getCb_id()%>"> <input type="hidden" name="pageNum" value="<%=pageNum%>"> <input type="hidden" name="cb_cmt_writer" value="<%=userid%>">
			...
			<form name="chanban_updateCmt" method="post" action="chanban_contentPro.jsp">
				...
				<div class="form-group row" style="border-left: 5px solid blue;">
					<label class="col-sm-2 col-form-label text-center"> <img src="/movie-dam/assets/img/profile-img/<%=writer_profile.getMem_img()%>" width="128">
					</label>
					<div class="col-sm-10">
						<%=comment.getCb_cmt_writer()%>님의 <span style="color: blue"><%=comment.getCb_cmt_type()%></span> 의견 -
						<%=comment.getCb_cmt_date()%><br>
				...
				<div class="form-group row text-left">
					...
					<div class="col-sm-9">
						<%=comment.getCb_cmt_writer()%>님의 의견 -
						<%=comment.getCb_cmt_date()%><br>
						<%=comment.getCb_cmt_content()%>
					</div>
				</div>
				...
				<div class="form-group row text-right" style="border-right: 5px solid red">
					<div class="col-sm-10">
						<%=comment.getCb_cmt_writer()%>님의 <span style="color: red"><%=comment.getCb_cmt_type()%></span> 의견 -
						<%=comment.getCb_cmt_date()%><br>
						<%=comment.getCb_cmt_content()%>
					</div>
					<label class="col-sm-2 col-form-label text-center"> <img src="/movie-dam/assets/img/profile-img/<%=writer_profile.getMem_img()%>" width="128">
				...
				<div class="form-group row text-right">
					...
					<label class="col-sm-3 col-form-label text-center"> <img src="/movie-dam/assets/img/profile-img/<%=writer_profile.getMem_img()%>" width="128"> <┚
					</label>
				</div>
				<%
					}
				...
				<div class="col-sm-12 text-right">
					...
					%>
					<a class="btn btn-sm btn-outline-secondary" onclick="this.nextSibling.style.display=(this.nextSibling.style.display=='none')?'block':'none';" href="javascript:void(0)"> 답글쓰기 </a>
					<div style="display: none">

						<input type="hidden" name="cb_cmt_id" value="<%=comment.getCb_cmt_id()%>"> <input type="hidden" name="cb_cmt_group" value="<%=comment.getCb_cmt_group()%>"> <input type="hidden" name="cb_cmt_step" value="<%=comment.getCb_cmt_step()%>"> <input type="hidden" name="cb_cmt_level" value="<%=comment.getCb_cmt_level()%>"> <input type="hidden" name="cb_cmt_type" value="<%=comment.getCb_cmt_type()%>"> <input type="hidden" name="cb_cmt_ref" value="<%=cb_id%>"> <input type="hidden" name="pageNum" value="<%=pageNum%>"> <input type="hidden" name="cb_cmt_writer" value="<%=userid%>"> <input type="hidden" name="userid" value="<%=userid%>">
