		...
		<div class="row justify-content-center">
			<div class="col-12">
				<form method="post" name="writeForm" action="chanban_updatePro.jsp?pageNum=<%=pageNum%>&cb_id=<%=cb_id%>" onsubmit="return writeSave()" enctype="multipart/form-data">
					<input type="hidden" name="cb_id" value="<%=cb_id%>"> <input type="hidden" id="inputWriter" name="cb_writer" value="<%=userid%>" readonly="readonly" required="required">
					...
					<div class="form-group row mb-3">
						...
						<div class="custom-file col-sm-10">
							<div class="custom-control custom-radio custom-control-inline">
								<input type="radio" id="inputCbtype_chan" name="cb_type" class="custom-control-input" value="찬성" <%if (chanban.getCb_type().equals("찬성")) {
				out.print(" checked=\"checked\"");
			}%>> <label class="custom-control-label" for="inputCbtype_chan">찬성</label>
							</div>
							<div class="custom-control custom-radio custom-control-inline">
								<input type="radio" id="inputCbtype_ban" name="cb_type" class="custom-control-input" value="반대" <%if (chanban.getCb_type().equals("반대")) {
				out.print(" checked=\"checked\"");
