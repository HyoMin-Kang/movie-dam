	...
	<div class="container">
			...
			<div class="col-12 col-sm-6 col-lg-4">
				<div class="single-features-area mb-50">
					<a href="content_rest.jsp?article_id=<%=article.getArticle_id()%>&pageNum=<%=currentPage%>&board_id=2&theater=<%=theater%>">
						<img style="width: 100%; height: 250px; display: block;" src="/movie-dam/imageFolder/cinema_restaurant/<%=article.getArticle_file()%>" alt="저장된 사진이 없습니다.">
					</a>
					<div class="feature-content d-flex align-items-center justify-content-between">
						<div class="feature-title">
							<h5>
								...
								<a href="content_rest.jsp?article_id=<%=article.getArticle_id()%>&pageNum=<%=currentPage%>&board_id=2&theater=<%=theater%>">
									<br>
									<%=article.getArticle_title()%><small> (<%=ccount%>)
									</small>
								</a>
							</h5>
		...
		<nav aria-label="Page navigation">
			<ul class="pagination justify-content-center">
				...
				%>
				<c:if test="${option eq null || search eq null}">
					<li class="page-item active"><a class="page-link" href="cinema_restaurant.jsp?pageNum=<%=i%>&theater=<%=theater%>"><%=i%>
							<span class="sr-only">(current)</span>
						</a></li>
				</c:if>
				<c:if test="${option ne null || search ne null}">
					<li class="page-item active"><a class="page-link" href="cinema_restaurant.jsp?pageNum=<%=i%>&theater=<%=theater%>&option=<%=option%>&search=<%=search%>"><%=i%>
							<span class="sr-only">(current)</span>
						</a></li>
				</c:if>
